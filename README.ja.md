...
現在の MVP には、デフォルト無効でプロバイダー非依存の提案マネージャーが
ありますが、本番用 AI プロバイダーアダプターはありません。コマンド実行や
パッチ書き込みもできません。目標は、無制限のシェルを持つ自律エージェント
ではなく、将来のアダプターが人間の統制する調整レイヤーで次の作業を支援
することです。
...
それらを送信しません。
実装済みの提案専用コアと、現在 `NOT RUN` のプロバイダー境界は
[`docs/AGENT_PROPOSALS.md`](docs/AGENT_PROPOSALS.md) に記載しています。

## オープンソース計画

- **現在:** 最初のプレリリース候補を固定して検証し、すべての外部統合の
  主張を日付付きエビデンスに結び付けます。
- **次:** リリースチェックリストを完了し、限定されたパイロット報告を
  収集して、範囲の明確なコントリビューターレビューを反映します。
...
- プロトコル v1 のリクエスト・SSE エンベロープ、再起動後も読める
  ジョブプロジェクション、中断監査記録の安全な終結
- 復元ジョブのログ・成果物取得と、完了ジョブのレコード・ワークスペース・
  ログ・成果物スナップショットをまとめて消す明示的削除
- 認証付き成果物ダウンロード
- リレー所有の成果物スナップショットと任意の専用キー HMAC-SHA256 マニフェスト
- 固定プリセット手順向けの任意のダイジェスト固定・非root・ネットワーク遮断
  コンテナ境界（契約テスト済み、実デーモン実行はこの環境で `NOT RUN`）
- 外部パッケージ不要の同梱デモ
- Node.js、Android Gradle、CMake プリセット
- 子プロセス環境変数の許可リストと防御的な秘密情報のマスキング
- 実行中プロセスと成果物確定を待つ正常終了
- 正確なターゲット・ref 許可リスト、有効期限付き承認、状態・キャンセル
  API、制限付き ZIP エビデンス取得、明示的な完了実行削除を備えた任意の GitHub Actions アダプター
...
- 実際の GitHub Actions キャンセルと非公開リポジトリアクセス

自動テストは Actions・Android アダプター契約、認証 API、モバイル UI
契約、翻訳の同等性を検査します。許可リストの公開リポジトリワークフロー
1件は実際に起動・監視し、ログ・エビデンス ZIP をリレー経由で取得して
ダイジェストを検証しました。実際のキャンセルと Android SDK・デバイス検査は
`NOT RUN` です。
