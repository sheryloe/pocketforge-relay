...
현재 MVP에는 기본 비활성·공급자 중립 제안 관리자가 있지만 운영용 AI 공급자
어댑터는 없습니다. 명령을 실행하거나 패치를 쓸 수도 없습니다. 목표는 제한
없는 셸을 가진 자율 에이전트가 아니라, 앞으로 구현할 어댑터가 사람이 통제하는
조율 계층에서 다음 일을 돕도록 하는 것입니다.
...
구현된 제안 전용 코어와 현재 `NOT RUN`인 공급자 경계는
[`docs/AGENT_PROPOSALS.md`](docs/AGENT_PROPOSALS.md)에 설명되어 있습니다.

## 오픈 소스 계획

- **현재:** 첫 사전 릴리스 후보를 고정하고 검증하며, 모든 외부 통합 주장을
  날짜가 있는 증거에 연결합니다.
- **다음:** 릴리스 체크리스트를 완료하고 제한된 파일럿 보고서를 수집하며,
  범위가 명확한 기여자 검토를 반영합니다.
...
- 정확한 대상·ref 허용 목록, 만료 승인, 상태·취소 API, 제한된 ZIP 증거
  다운로드와 명시적인 완료 실행 삭제를 갖춘 선택적 GitHub Actions 어댑터
